...
router.get('/', async (req, res) => {
  try {
    ...
    // Filter by announcement type (NOTICE, COMMITTEE, EVENT, TIMETABLE, EMERGENCY) if provided
    const categoryParam = req.query.category || req.query.type;
    if (categoryParam && typeof categoryParam === 'string' && ['NOTICE', 'COMMITTEE', 'EVENT', 'TIMETABLE', 'EMERGENCY'].includes(categoryParam.trim().toUpperCase())) {
